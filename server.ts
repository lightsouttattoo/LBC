...
async function startServer() {
  ...
  // API Route: Baptist Pastor AI Virtual Faith Assistant endpoint
...
7. Keep response lengths balanced for chat (typically 2 to 4 paragraphs or message + prayer).`;
        ...
        if (Array.isArray(conversationHistory) && conversationHistory.length > 0) {
          const recent = conversationHistory.slice(-8).map((msg: any) => 
            `${msg.isUser ? "Believer" : name}: ${msg.content}`
          ).join("\n");
    ...
    // Fallback intelligent response if API key is not present or API call fails
    const mockPastor = {
      id: 'pastor',
      name,
      role,
